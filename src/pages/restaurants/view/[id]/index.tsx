...
import { deleteTableStatusById } from 'apiSdk/table-statuses';
...
function RestaurantViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<RestaurantInterface>(
    ...
    () =>
      getRestaurantById(id, {
        relations: ['user', 'menu_item', 'table_status'],
      }),
  );
  ...
  const table_statusHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('table_status', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/table-statuses/create?restaurant_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.table_status?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/table-statuses/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            ...
                            <Td>
                              {hasAccess('table_status', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/table-statuses/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('table_status', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    table_statusHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
